...
data_table <- data.frame(table(data$sex, data$pdsi_avg, data$reasons_for_migration_environmetal, data$ADM1_NAME, 
...
ggplot(data_table, aes(x=drought_severity, y=number_of_migrants, color=sex)) + geom_point() 
...
ggplot(data_table, aes(x=drought_severity, y=number_of_migrants, color = sex)) + geom_point() + geom_vline(xintercept = 0, linetype = 2, size = 0.5, color = "red") + facet_grid(.~somali)
